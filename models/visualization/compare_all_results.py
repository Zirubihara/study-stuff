...
def load_results():
    ...
    tensorflow_file = "../results/tensorflow_anomaly_detection_results.json"
    if Path(tensorflow_file).exists():
        with open(tensorflow_file, 'r') as f:
            results['tensorflow'] = json.load(f)

    # Load JAX results
    jax_file = "../results/jax_anomaly_detection_results.json"
    if Path(jax_file).exists():
        with open(jax_file, 'r') as f:
            results['jax'] = json.load(f)

    # Load XGBoost results
    xgboost_file = "../results/xgboost_anomaly_detection_results.json"
    if Path(xgboost_file).exists():
...
def create_comparison_charts(results, output_dir="../charts"):
    ...
    if 'tensorflow' in results:
        tensorflow_data = results['tensorflow']['tensorflow_autoencoder']
        models.append('TensorFlow\nAutoencoder')
        train_times.append(tensorflow_data['training_time'])
        inference_times.append(tensorflow_data['inference_time'])
        anomalies.append(tensorflow_data['n_anomalies'])
        inference_speeds.append(tensorflow_data['inference_speed'])
        memory_usage.append(tensorflow_data['memory_usage_gb'])

    # JAX model
    if 'jax' in results:
        jax_data = results['jax']['jax_autoencoder']
        models.append('JAX\nAutoencoder')
        train_times.append(jax_data['training_time'])
        inference_times.append(jax_data['inference_time'])
        anomalies.append(jax_data['n_anomalies'])
        inference_speeds.append(jax_data['inference_speed'])
        memory_usage.append(jax_data['memory_usage_gb'])

    # XGBoost model
    if 'xgboost' in results:
        xgboost_data = results['xgboost']['xgboost_detector']
        models.append('XGBoost')
        train_times.append(xgboost_data['training_time'])
        inference_times.append(xgboost_data['inference_time'])
        anomalies.append(xgboost_data['n_anomalies'])
        inference_speeds.append(xgboost_data['inference_speed'])
        memory_usage.append(xgboost_data['memory_usage_gb'])
