#!/usr/bin/env python3
"""
Run benchmarks for all 5 libraries on 5M dataset only
"""
...
def run_5m_benchmark():
    """Run benchmarks on 5M dataset for all libraries"""

    # Only 5M dataset
    dataset_config = {"rows": 5000000, "name": "5M", "file": "../../data/benchmark_5m.csv"}
    ...
    print(f"{'=' * 60}")
    print("TESTING 5M DATASET BENCHMARK")
    print(f"{'=' * 60}")
    print(f"Libraries: {', '.join(libraries)}")
    print(f"Dataset: 5M rows")
    ...
    for lib_name in libraries:
        ...
        try:
            ...
            result = process_implementation_with_custom_path(
                processor, f"{lib_name}_5M", num_runs, warmup_runs
            )
            if result:
    ...
    # Print summary
    print(f"\n{'=' * 60}")
    print("5M DATASET RESULTS SUMMARY")
    print(f"{'=' * 60}")
