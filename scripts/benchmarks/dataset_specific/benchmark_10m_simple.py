#!/usr/bin/env python3
"""
Run benchmarks for all 5 libraries on 10M dataset only
"""
...
def run_10m_benchmark():
    """Run benchmarks on 10M dataset for all libraries"""

    # Only 10M dataset
    dataset_config = {"rows": 10000000, "name": "10M", "file": "../../data/benchmark_10m.csv"}
    ...
    print(f"{'=' * 60}")
    print("TESTING 10M DATASET BENCHMARK")
    print(f"{'=' * 60}")
    print(f"Libraries: {', '.join(libraries)}")
    print(f"Dataset: 10M rows")
    ...
    for lib_name in libraries:
        ...
        try:
            ...
            result = process_implementation_with_custom_path(
                processor, f"{lib_name}_10M", num_runs, warmup_runs
            )
            if result:
    ...
    # Print summary
    print(f"\n{'=' * 60}")
    print("10M DATASET RESULTS SUMMARY")
    print(f"{'=' * 60}")
    ...
    print(f"\nFiles saved to: ../../results/performance_metrics_<library>_10M.json")
    return results
