...
def run_technology(script_path, technology_name):
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"SUCCESS: {technology_name.upper()} completed successfully in {execution_time:.2f} seconds")
...
def main():
    ...
    print("Note: PySpark 5M results already available in performance_metrics_spark_5m.json")
    ...
    for tech_name, result in results.items():
        ...
        print(f"{tech_name.upper():10} | {status:12} | {result['execution_time']:8.2f}s")
        if result['success']:
            successful_runs += 1
