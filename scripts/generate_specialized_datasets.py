...
def generate_pandas_showcase_dataset(output_path: str, num_rows: int = 500000):
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        # Header
        writer.writerow([
            "employee_id", "full_name", "email", "phone", "hire_date", "birth_date",
            "department", "city", "salary", "bonus", "status", "notes", "skills",
            "last_login", "project_hours", "rating"
        ])
        ...
        for i in range(num_rows):
            ...
            # Complex text data
            notes = f"Employee in {department}. " + \
            ...
            all_skills = ["Python", "SQL", "Excel", "PowerBI", "Tableau", "R", "Java", "JavaScript"]
            skills = json.dumps(random.sample(all_skills, random.randint(1, 4)))
            
            # Recent datetime
            last_login = datetime.now() - timedelta(days=random.randint(0, 30)) if random.random() > 0.1 else None
...
def generate_pyarrow_showcase_dataset(output_path: str, num_rows: int = 5000000):
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        writer.writerow([
            "transaction_id", "timestamp", "user_id", "product_id", "category_id",
            "quantity", "unit_price", "total_amount", "discount", "tax",
            "region_id", "channel_id", "is_premium", "customer_score"
        ])
...
def generate_polars_showcase_dataset(output_path: str, num_rows: int = 2000000):
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        writer.writerow([
            "device_id", "timestamp", "sensor_type", "measurement_value",
            "location_lat", "location_lon", "temperature", "humidity",
            "battery_level", "signal_strength", "error_code", "quality_score"
        ])
        ...
        for i in range(num_rows):
            ...
            humidity = round(max(0, min(100, 50 + (25 - base_temp) * 2 + random.gauss(0, 5))), 1)
            ...
            writer.writerow([
                device_id, timestamp.strftime("%Y-%m-%d %H:%M:%S"), sensor_type,
                measurement_value, location_lat, location_lon, temperature,
                humidity, round(battery_level, 1), round(signal_strength, 1),
...
def generate_dask_showcase_dataset(output_path: str, num_rows: int = 10000000):
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for i in range(num_rows):
            ...
            measurement_5 = round(measurement_1 * measurement_2 / 100 + random.gauss(0, 3), 4)
            measurement_6 = round(np.sqrt(abs(measurement_3)) * 10 + random.gauss(0, 2), 4)
...
def generate_spark_showcase_dataset(output_path: str, num_rows: int = 20000000):
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        writer.writerow([
            "customer_id", "transaction_date", "product_category", "product_subcategory",
            "brand_id", "store_id", "sales_amount", "quantity", "discount_amount",
            "customer_age", "customer_income", "customer_segment", "geography_region",
            "marketing_channel", "is_weekend", "season", "feature_1", "feature_2",
            "feature_3", "feature_4", "target_variable"
        ])
        
        # Large-scale retail/enterprise data
        categories = ["Electronics", "Clothing", "Home", "Sports", "Books", "Beauty", "Food"]
        subcategories = {
        ...
        for i in range(num_rows):
            ...
            transaction_date = base_date + timedelta(days=random.randint(0, 1460))  # 4 years
            ...
            target_variable = 1 if (sales_amount > 100 and customer_income > 50000) else 0
            
            writer.writerow([
                customer_id, transaction_date.strftime("%Y-%m-%d"), category, subcategory,
                brand_id, store_id, sales_amount, quantity, discount_amount,
                customer_age, customer_income, customer_segment, geography_region,
                marketing_channel, is_weekend, season, feature_1, feature_2,
                feature_3, feature_4, target_variable
