...
def modify_script_for_50m(script_path, technology_name):
    ...
    if technology_name == 'dask':
        content = content.replace(
            ...
            'Client(processes=False, memory_limit="8GB")'
        )
...
def run_technology(script_path, technology_name):
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"SUCCESS: {technology_name.upper()} completed in {execution_time:.2f} seconds ({execution_time/60:.1f} minutes)")
        ...
        print(f"TIMEOUT: {technology_name.upper()} timed out after {timeout/60:.0f} minutes")
...
def main():
    ...
    print("Expected times: Polars ~30s, PyArrow ~2min, Pandas ~5min, Dask ~10min, Spark ~4min")
    print("=" * 80)
    ...
    print("Note: PySpark 50M results already available in performance_metrics_spark_50m.json")
