...
def pandas_strength_benchmark():
    ...
    df['bonus_filled'] = df['bonus'].fillna(df.groupby(['department', 'city'])['bonus'].transform('mean'))
...
def polars_strength_benchmark():
    ...
    optimized_result = (
        ...
        .with_columns([
            ...
            pl.when(pl.col("battery_level") < 20).then(pl.lit("critical"))
             .when(pl.col("battery_level") < 50).then(pl.lit("low"))
             .otherwise(pl.lit("normal")).alias("battery_status")
        ])
        .filter(
    ...
    windowed = ts_df.with_columns([
        pl.col("measurement_value").rolling_mean(window_size=100).alias("rolling_avg"),
