...
def run_benchmark(script, dataset_size, dataset_path):
    ...
    if '1m' in dataset_size.lower():
        new_content = content.replace('csv_path = large_dataset', f'csv_path = "{dataset_path}"')
        new_content = new_content.replace('csv_path = massive_dataset', f'csv_path = "{dataset_path}"')
        new_content = new_content.replace('csv_path = medium_dataset', f'csv_path = "{dataset_path}"')
    else:
        new_content = content.replace('csv_path = large_dataset', f'csv_path = "{dataset_path}"')
        new_content = new_content.replace('csv_path = massive_dataset', f'csv_path = "{dataset_path}"')
        new_content = new_content.replace('csv_path = medium_dataset', f'csv_path = "{dataset_path}"')
