...
def run_technology(script_path, technology_name):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(
                f"SUCCESS: {technology_name.upper()} completed in {execution_time:.2f}s ({minutes:.1f} minutes)"
            )
            print(
                "Output:",
        ...
        print(
            "This suggests the technology may not be suitable for datasets this large"
        )
...
def main():
    ...
    for tech_name, script_path in technologies.items():
        ...
        print(
            f"If this fails, it likely means {tech_name} cannot handle 3GB datasets efficiently"
        )
