#!/usr/bin/env python3
"""
Run benchmarks for all 5 libraries on 50M dataset only
"""
...
def run_50m_benchmark():
    """Run benchmarks on 50M dataset for all libraries"""

    # Only 50M dataset
    dataset_config = {"rows": 50000000, "name": "50M", "file": "../../data/benchmark_50m.csv"}
    ...
    print(f"{'=' * 60}")
    print("TESTING 50M DATASET BENCHMARK")
    print(f"{'=' * 60}")
    print(f"Libraries: {', '.join(libraries)}")
    print(f"Dataset: 50M rows")
    ...
    for lib_name in libraries:
        ...
        try:
            ...
            result = process_implementation_with_custom_path(
                processor, f"{lib_name}_50M", num_runs, warmup_runs
            )
            if result:
    ...
    # Print summary
    print(f"\n{'=' * 60}")
    print("50M DATASET RESULTS SUMMARY")
    print(f"{'=' * 60}")
    ...
    print(f"\nFiles saved to: ../../results/performance_metrics_<library>_50M.json")
    return results
