...
def chart_1_total_execution_time():
    ...
    plt.suptitle(
        "Total Execution Time Comparison", fontsize=20, fontweight="bold", y=0.98
    )
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig(
        "../charts/01_total_execution_time.png",
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
    )
...
def chart_2_loading_time_detailed():
    ...
    ax.set_title(
        "CSV Loading Performance Comparison", fontsize=18, fontweight="bold", pad=20
    )
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(
        "../charts/02_loading_time_comparison.png",
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
    )
...
def chart_3_memory_usage_simple():
    ...
    for library in libraries:
        ...
        if "100K" in data[library]:
            peak_mem = max(
                data[library]["100K"].get(f"{op}_memory_mean", 0) for op in operations
            )
        ...
        if "500K" in data[library]:
            peak_mem = max(
                data[library]["500K"].get(f"{op}_memory_mean", 0) for op in operations
            )
    ...
    plt.tight_layout()
    plt.savefig(
        "../charts/03_memory_usage_comparison.png",
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
    )
...
def chart_4_aggregation_performance():
    ...
    ax.set_title(
        "Group-By Aggregation Performance", fontsize=18, fontweight="bold", pad=20
    )
    ax.set_xticks(x)
    ax.set_xticklabels([lib.capitalize() for lib in libraries])
    ax.legend(loc="upper right", frameon=True, fancybox=True, shadow=True)
    ...
    plt.tight_layout()
    plt.savefig(
        "../charts/04_aggregation_performance.png",
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
    )
...
def chart_5_scalability_lines():
    ...
    for i, library in enumerate(libraries):
        ...
        for size_label in size_labels:
            if size_label in data[library]:
                ...
                total_time = sum(
                    data[library][size_label].get(f"{op}_time_mean", 0)
                    for op in operations
                )
                total_times.append(total_time)
            else:
    ...
    plt.tight_layout()
    plt.savefig(
        "../charts/05_scalability_analysis.png",
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
    )
...
def chart_6_operation_ranking():
    ...
    for i, op in enumerate(operations):
        ...
        ax.set_title(
            f"{op.capitalize()} Operation", fontweight="bold", pad=15, fontsize=16
        )
    ...
    plt.suptitle(
        "Individual Operation Performance Ranking (500K Dataset)",
        fontsize=20,
        fontweight="bold",
        y=0.98,
    )
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig(
        "../charts/06_operation_rankings.png",
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
    )
