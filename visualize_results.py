...
def create_scalability_analysis():
    ...
    for i, library in enumerate(libraries):
        ...
        if len(total_times) == len(sizes):
            ax1.errorbar(size_values, total_times, yerr=total_stds, 
    ...
    for i, library in enumerate(libraries):
        ...
        for size in sizes:
            if size in data[library]:
                ...
                operations = ['loading', 'cleaning', 'aggregation', 'sorting', 'filtering', 'correlation']
                peak_mem = max(data[library][size].get(f"{op}_memory_mean", 0) for op in operations)
                peak_memory.append(peak_mem)
        
        if len(peak_memory) == len(sizes):
            ax2.plot(size_values, peak_memory, marker='o', linewidth=2, markersize=8,
                    label=library.capitalize(), color=colors[i % len(colors)])
...
def create_statistical_summary_table():
    ...
    for size in ['100K', '500K']:
        ...
        print("-" * 80)
        print(f"{'Library':<10} {'Total Time (s)':<15} {'±STD':<10} {'Peak Memory (MB)':<20} {'Fastest Op':<15}")
        print("-" * 80)
        ...
        for library in data.keys():
            if size in data[library]:
                ...
                # Find peak memory
                peak_memory = max(data[library][size].get(f"{op}_memory_mean", 0) for op in operations)
                
                # Find fastest operation
                op_times = {op: data[library][size].get(f"{op}_time_mean", float('inf')) for op in operations}
                fastest_op = min(op_times.keys(), key=lambda x: op_times[x])
                
                print(f"{library.capitalize():<10} {total_time:<15.3f} {total_std:<10.3f} {peak_memory:<20.1f} {fastest_op.capitalize():<15}")
